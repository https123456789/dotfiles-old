...
# Add bsos cross compilers
if test -d $HOME/coding/bsos/.cross_compiler/bin; then
    export PATH="$HOME/coding/bsos/.cross_compiler/bin:$PATH"
fi
...
## SSH Stuff
function get_rpi_ip() {
    nmap --privileged -p 22 192.168.1.0/24 -sS \
        | grep "Pi" -B 5 \
        | grep "Nmap scan report for 192.168.1." \
        | grep --color=never -Pom 1 '[0-9.]{7,15}' \
        | tr -d '\n'
}

function ssh_into_rpi() {
    echo "Getting Raspberry Pi IP..."
    rpi_ip=$(get_rpi_ip)
    echo "SSHing into machine ($rpi_ip)..."
    ssh $1@$rpi_ip
}
